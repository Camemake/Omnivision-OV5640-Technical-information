...
When paired with an application processor like the MediaTek MT6573, these functions are handled entirely by the **host ISP**, with the OV5640 delivering raw Bayer data. The ISP tuning lives in the AP's own register/NVRAM space. The `.h` files in the ini examples archive document exactly these host-side parameters, which is why they are absent from the OmniVision datasheet.
...
**[Camemake](https://camemake.eu)**

→ [camemake.eu](https://camemake.eu)
